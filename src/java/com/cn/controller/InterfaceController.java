...
//        System.out.println("where SQL:" + whereSql);
    ...
    public String batchUpdateField(String beanPackage, String tableName, String whereAry, String fieldName, String fieldValue, String connType) throws Exception {
        ...
        for (int i = 0; i < updateWhereAry.size(); i++) {
            JSONObject whereObj = updateWhereAry.getJSONObject(i);
            JSONObject setObj = new JSONObject();
            setObj.put(fieldName, fieldValue);
            updateStr.append(setObj.toJSONString());
            updateStr.append(",");
            updateStr.append(whereObj.toJSONString());
            updateStr.append(",");
        }
        ...
        ArrayList<Integer> result = commonController.dataBaseOperate(updateStr.toString(), beanPackage, tableName, "update", conn);
        if (result.get(0) == 0) {
            return Units.objectToJson(0, "操作成功", null);
        }
        return Units.objectToJson(-1, "操作失败!", null);
    }
