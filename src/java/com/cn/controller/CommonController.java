...
public class CommonController {
                ...
                for (Method method : methods) {
                    ...
                    if (methodName.startsWith("set") && !Modifier.isStatic(method.getModifiers())) {
                        ...
                        if (Units.isExistColumn(set, columnName)) {
                            // 得到方法的参数类型
                            Type[] parmts = method.getGenericParameterTypes();
    ...
    /**
     * 数据库查询操作
     * @param tableName 查询的数据库表名
     * @param className 返回数据对应的类名
     * @param fields
     * @param wherecase
     * @param pageSize
     * @param pageIndex
     ...
     * @throws Exception 
     */
    public List<Object> dataBaseQuery(String tableName, String className, String fields, String wherecase, int pageSize, int pageIndex, String orderField, int orderFlag,
            String connType) throws Exception {
        //System.out.println("wherecase:" + wherecase);
        CallableStatement statement = null;
        DatabaseOpt opt = new DatabaseOpt();
...
//            JSONArray array = JSONObject.parseArray(datas);
     ...
     * varchar(20)),则参数为: {"params1": "int,params1Val", "params2":
     * "string,params2Val"}
     * @param className
     * @param connType
     * @return
...
//        System.out.println("where date sql:" + result);
     ...
     * @param objClass
     * @param fieldName
     * @param isContainsAuto 是否包含auto类型的字段
     * @return 
     */
    public boolean isInput(Class objClass, String fieldName, boolean isContainsAuto) {
        Field[] fields = objClass.getDeclaredFields();
        for (Field field : fields) {
            if (field.getName().compareToIgnoreCase(fieldName) == 0) {
                if (field.isAnnotationPresent(FieldDescription.class)) {
                    FieldDescription description = field.getAnnotation(FieldDescription.class);
                    if (isContainsAuto) {
                        //注释的操作为input或import或auto的字段
                        if (description.operate().compareTo("input") == 0
                                || description.operate().compareTo("import") == 0
                                || description.operate().compareTo("auto") == 0) {
                            return true;
                        }
                    } else {
                        //注释的操作为input或import的字段
