(function ($) {
    var _funs_ = {
        getDOM: function () {
            ...
            //加载表头
            var result = '', txt = '', type = '', content = '', width = '', totalWidth = 0;
            ...
            for (var i in this.titles) {
                ...
                if (width === "0")
                    result += '<div class="col" name="' + i + '" style="display:none;"><h5>' + txt + '</h5><div class="inner"><ul class="' + type + '"></ul></div></div>';
                else
                    result += '<div class="col" name="' + i + '" style="width:' + width + ';"><h5>' + txt + '</h5><div class="inner"><ul class="' + type + '"></ul></div></div>';
                if (i === "checkbox") {
        ...
        getTableDataDOM: function (datas) {
            ...
            for (var i = 0; i < datas.length; i++) {
                ...
                for (var k in this.titles) {
                    var ul = this.$container.children("div[name='" + k + "']").find("ul");
                    var ulClass = ul.attr("class");
                    
                    if (k === "checkbox") {
                        ul.append("<li><input type='checkbox' index='" + i + "'/></li>");
                        continue;
                    }
                    
                    var content = '';
                    if (this.dataType[k]) {
                        content = getObjFieldsStr(obj, this.dataType[k].content);
                    }
                    if (obj[k]) {
                        if (ulClass === "map")
                            ul.append("<li content='" + content + "'>" + obj[k] + "</li>");
                        else if (ulClass === "img")
                            ul.append("<li><a>" + obj[k] + "</a><div id='" + obj[k] + "' style='display: none;'><img src='upload/" + obj[k] + ".jpg'/></div></li>");
                        else
                            ul.append("<li content='" + content + "'>" + obj[k] + "</li>");
                    } else {
                        ul.append("<li>" + "  " + "</li>");
        ...
        getPageInfo: function () {
            ...
            this.find(".jtb-page .page-info").html("当前第" + this.pageIndex + "页,当前页" + this.datas.length + "条数据,   共" + this.dataCount + "条数据");
            if (this.pageCount > 1) {
        ...
        bindEvt: function () {
            var that = this;
            this.$container.find("ul[class='txt']").on("click", "li", function (e) {
            ...
            this.$container.find("ul[class='map']").on("click", "li", function (e) {
                displayMapLayer(2, $(this).attr('content'), "地图");
            });
            
            this.$container.find("ul[class='mapPolygon']").on("click", "li", function (e) {
                displayMapDrawLayer(2, $(this).attr('content'), 'polygon', "绘制区域");
            });
        ...
        isUnique: function (obj) {
            for (var i = 0; i < this.datas.length; i++) {
                ...
                for (var e = 0; e < this.unique.length; e++) {
                    ...
                    if (!obj[key] && this.datas[i][key] == obj[key]) {
                ...
                for (var index = 0; index < this.primary.length; index++) {
                    ...
                    if (!obj[pri] || this.datas[i][pri] != obj[pri]) {
