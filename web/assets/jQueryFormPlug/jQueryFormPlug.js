...
$.fn.insertInputForm = function (options) {
    ...
    var _pro_ = {
        getDOM: function () {
            ...
            for (var i in controls) {
                ...
                if (controls[i].indexOf('@') >= 0) {
                    ...
                    if (controls[i].indexOf("@table") >= 0) {
                        inputResult += "<div><span>" + txt + "</span>:<input class='event input' name='" + i + "'/></div>";
                    } else if (controls[i].indexOf("@mulOptionTable") >= 0) {
                        inputResult += "<div><span>" + txt + "</span>:<input class='event input' name='" + i + "'/></div>";
                    } else if (controls[i].indexOf("@mapDraw") >= 0) {
                        inputResult += "<div><span>" + txt + "</span>:<input class='event input' name='" + i + "'/></div>";
                    } else {
        ...
        EvtInputFocus: function () {
            ...
            this.$inputs.filter(".event").on('click', function () {
                ...
                if (obj[name].indexOf("@table") >= 0) {
                    that.tableInput(that, obj, $input);
                } else if (obj[name].indexOf("@mulOptionTable") >= 0) {
                    that.mulOptionTableInput(that, obj, $input);
                } else if (obj[name].indexOf("@mapDraw") >= 0) {
                    that.mapDrawInput(that, obj, $input);
                }
            });
        ...
        tableInput: function (that, obj, $input) {
            var name = $input.attr("name");
            var arr = obj[name].split(':')[0].split(',');
            var relys = arr.slice(2).map(function (i) {
                return i.slice(1);
            });
            var $rely, rely_obj = {};
...
//                    console.log(obj[name]);
        ...
        mulOptionTableInput: function (that, obj, $input) {
            var name = $input.attr("name");
            var arr = obj[name].split(':')[0].split(',');
            var relys = arr.slice(2).map(function (i) {
                return i.slice(1);
        ...
        mapDrawInput: function (that, obj, $input) {
            var name = $input.attr("name");
            var arr = obj[name].split(':')[0].split(',');
            var relys = arr.slice(2).map(function (i) {
                return i.slice(1);
            ...
            //显示地图绘制界面
            displayMapDrawLayer(2, rely_obj, 'draw', "绘制区域", function(polygonName, polygonPath) {
                $input.val(polygonName);
                that.$inputs.filter("[name='polygonPath']").val(polygonPath);
            });
        },
